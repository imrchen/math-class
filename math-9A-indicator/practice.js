    ...
    const cell = (no, tag) => {
      ...
      return `<div style="border:1.5px solid #dbe3f0;border-radius:10px;background:#fff;
        ...
        <span style="min-width:0;flex:1">
          <span style="font-size:19px;font-weight:700;color:${miss ? '#e11d48' : ANS};
            display:block;line-height:1.35;word-break:break-word">${miss ? '（查無答案）' : big}</span>
          ${sub ? `<span style="font-size:13.5px;color:${GREY};display:block;line-height:1.4">${sub}</span>` : ''}
        </span>
      </div>`;
    };
    const block = (g) => `<div>
      <div style="font-size:14px;font-weight:700;color:#2563eb;margin:0 0 6px 2px">${g.label}</div>
      <div style="display:grid;grid-template-columns:repeat(${g.cols || 3},minmax(0,1fr));gap:7px">
